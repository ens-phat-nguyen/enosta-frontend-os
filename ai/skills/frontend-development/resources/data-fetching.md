...
Create centralized API service per module:

```
modules/
  my-feature/
    api/
      my-feature-api.ts    # Service layer
```
 ...
 * Uses apiClient from core layer for consistent error handling
 */
...
// apiClient is a configured fetch/axios instance
...
**Do NOT create new API clients** - use the core apiClient for consistency.
...
1. **Create API Service**: `modules/X/api/x-api.ts` using core apiClient
...
5. **Error Handling**: onError + error boundaries
6. **Mutations**: invalidateQueries after success
7. **Type Safety**: Type all parameters and returns
